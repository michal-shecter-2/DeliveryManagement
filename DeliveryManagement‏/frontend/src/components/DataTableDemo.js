...
export default function DataTableDemo() {
    ...
    function getAllEvents() {
        return axios.get('http://localhost:5000/events').then(res => res.data);
    }
    useEffect(() => {
        getAllEvents().then(data => setEvents(data));
        // const filtertmp = tmp.filter((item) => item.id == 3)
        // setEvents(tmp);
    ...
    const TypeOfEventsBodyTemplate = (rowData) => {
        return (
            <React.Fragment>
                <span className="p-column-title">Date</span>
                <span>{rowData.typeofevent}</span>
            </React.Fragment>
        );
    }
        ...
        <div className="datatable-doc-demo">
            <div className="card">
                <DataTable ref={dt} value={events}
                    ...
                    <Column field="id" header="ID" body={IdBodyTemplate} sortable filter filterPlaceholder="Search by id" />
                    <Column field="customer" header="Customer" body={IdBodyTemplate} sortable filter filterPlaceholder="Search by customer" />
                    <Column field="type_of_events" header="type of events" body={TypeOfEventsBodyTemplate} sortable filter filterPlaceholder="Search by type of events" />
                    ...
                    <Column field="location" header="Location" body={LocationBodyTemplate} sortable filter filterPlaceholder="Search by location" />
